...
{
    private IUAVariable filterTextVariable;

    public override void Start()
	{
		pathVariable = LogicObject.GetVariable("Path");
        Log.Info(pathVariable.Value);
        ...
        // Initialize filterTextVariable
        filterTextVariable = Owner.GetVariable("FilterText");
        if (filterTextVariable == null)
            throw new CoreConfigurationException("FilterText variable not found in FilesystemBrowserLogic");
		...
		Log.Info(startFolderPathResourceUri);
	...
	private void Browse(ResourceUri resourceUri)
	{
		...
		var files = currentDirectory.GetFileSystemInfos().Where(entry => entry is FileInfo &&
																		 FileHasToBeListed(entry));
        // Get the filter text
        string filterText = filterTextVariable.Value;
        foreach (var file in files)
		{
            ...
            if (!string.IsNullOrEmpty(filterText) && !file.Name.Contains(filterText))
                continue;
...
namespace FilesystemBrowserHelper
{
	public class ResourceUriHelper
	{
		...
		public string GetFTOptixStudioFormattedPath(ResourceUri oldResourceUri,
			string updatedPath)
		{
